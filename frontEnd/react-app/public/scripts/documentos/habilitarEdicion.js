...
export function editar(){ 
  ...
  try{ 
    // Initialize the set to store BienIDs marked for deletion
    if(typeof window !== 'undefined' && !window.__bienesToDelete) window.__bienesToDelete = new Set();
    document.dispatchEvent(new CustomEvent('documento:editable'));
    // Enable removal of existing bienes now that we're in edit mode
    try{ enableExistingBienRemoval(); }catch(e){}
  }catch(e){}
}

// Enable remove buttons for existing bienes when entering edit mode
function enableExistingBienRemoval(){
  const container = document.getElementById('bienes-container'); if(!container) return;
  const groups = Array.from(container.querySelectorAll('div[data-index]'));
  groups.forEach(div => {
    const removeBtn = div.querySelector('.remove-bien');
    if(!removeBtn) return;
    // Try to read BienID from hidden input inside the group
    const hidden = div.querySelector('input[type="hidden"][name^="bien_id_"]');
    const bienId = hidden && hidden.value ? String(hidden.value).trim() : null;
    if(bienId){
      // Enable the button and replace the default handler which prevented deletion
      removeBtn.disabled = false;
      removeBtn.title = 'Eliminar esta propiedad (se guardará al guardar cambios)';
      // Remove previous listeners by cloning the node
      const newBtn = removeBtn.cloneNode(true);
      removeBtn.parentNode.replaceChild(newBtn, removeBtn);
      newBtn.addEventListener('click', ()=>{
        if(!confirm('¿Confirma eliminar esta propiedad? Esta acción se aplicará al guardar.')) return;
        try{ if(typeof window !== 'undefined'){ if(!window.__bienesToDelete) window.__bienesToDelete = new Set(); window.__bienesToDelete.add(Number(bienId)); } }catch(e){}
        // remove from DOM so it won't be sent in the save payload
        div.remove();
      });
    }
  });
...
async function guardarCambios(){
  ...
  const combinedDigitalIdent = digitalPref ? (digitalIdentificacion ? `${digitalPref}-${digitalIdentificacion}` : null) : (digitalIdentificacion || null);
      ...
      try{
        ...
        // Handle deletions: any BienID recorded in window.__bienesToDelete should be deleted on server
        try{
          if(typeof window !== 'undefined' && window.__bienesToDelete && window.__bienesToDelete.size){
            for(const bid of Array.from(window.__bienesToDelete)){
              if(!bid) continue;
              updates.push(jsonFetch(`/bienes/${bid}`,'DELETE'));
            }
            // clear the set after scheduling deletes
            window.__bienesToDelete.clear();
          }
        }catch(e){ /* ignore deletion scheduling errors */ }
